...
/** @param {File} file */
export function validateAnnotateInput(file) {
  ...
  if (file.type === 'application/pdf' || /\.pdf$/i.test(file.name || '')) {
    if (file.size > MAX_PDF_BYTES) {
      return {
        ok: false,
        code: 'too_large',
        message: `PDF は ${Math.round(MAX_PDF_BYTES / (1024 * 1024))}MB までです。`,
      };
    }
    return { ok: true, mime: 'application/pdf', kind: 'pdf' };
  }
  return {
    ok: false,
    code: 'bad_type',
    message: 'PNG / JPEG / WebP / GIF / PDF のみ対応です。',
  };
}
...
/** @param {string} sourceName @param {'png'|'jpeg'} [ext] */
export function outputFilenameExt(sourceName, ext = 'png') {
  const base = (sourceName || 'screenshot').split(/[/\\]/).pop() || 'screenshot';
  const stem = base.replace(/\.[^.]+$/, '') || 'screenshot';
  return ext === 'jpeg' ? `${stem}-annotated.jpg` : `${stem}-annotated.png`;
}
...
export function canvasToJpegBlob(canvas, quality = 0.92) {
  return new Promise((resolve, reject) => {
    canvas.toBlob(
      (blob) => {
        if (!blob) reject(new Error('JPEG の生成に失敗しました。'));
        else resolve(blob);
      },
      'image/jpeg',
      quality,
 ...
 * @typedef {{ id: string, type: 'ellipse', x: number, y: number, w: number, h: number }} EllipseShape
 * @typedef {{ id: string, type: 'text', x: number, y: number, text: string, fontSize?: number }} TextShape
 * @typedef {ArrowShape|RectShape|EllipseShape|TextShape} AnnotateShape
 */
...
export function drawRoundedFrame(ctx, x, y, w, h, selected = false) {
  ...
  const magentaW = Math.max(5, Math.round(6 * scale));
  const whiteW = magentaW + Math.max(2, Math.round(2.5 * scale));
  const r = Math.min(10 * scale, Math.max(4, Math.min(w, h) * 0.1));
 ...
 * @param {boolean} [selected]
 */
export function drawEllipseFrame(ctx, x, y, w, h, selected = false) {
  const { scale } = annotateStrokeWidths(ctx);
  const magentaW = Math.max(5, Math.round(6 * scale));
  const whiteW = magentaW + Math.max(2, Math.round(2.5 * scale));
  const cx = x + w / 2;
...
/** @param {TextShape} shape @param {boolean} [selected] */
export function drawTextLabel(ctx, shape, selected = false) {
  const { scale } = annotateStrokeWidths(ctx);
  const fontSize = shape.fontSize || Math.max(14, Math.round(16 * scale));
  ctx.save();
  ctx.font = `bold ${fontSize}px system-ui, sans-serif`;
  ...
  ctx.strokeStyle = selected ? ANNOTATE_MAGENTA : '#cbd5e1';
  ctx.lineWidth = selected ? 2 : 1;
  ctx.fillRect(shape.x, shape.y, tw, th);
  ctx.strokeRect(shape.x + 0.5, shape.y + 0.5, tw - 1, th - 1);
  ctx.fillStyle = '#1e293b';
  ctx.fillText(shape.text || '', shape.x + pad, shape.y + pad);
  ctx.restore();
}

/** @param {EllipseShape} shape */
export function hitTestEllipseFrame(shape, px, py, pad = ANNOTATE_HIT_PAD) {
  const handle = hitTestRoundedFrame(shape, px, py, pad);
  if (handle) return handle;
  const cx = shape.x + shape.w / 2;
  const cy = shape.y + shape.h / 2;
  const rx = Math.max(4, shape.w / 2);
  const ry = Math.max(4, shape.h / 2);
  const nx = (px - cx) / rx;
  const ny = (py - cy) / ry;
  if (nx * nx + ny * ny <= 1.05) return 'body';
  return null;
}

/** @param {TextShape} shape */
export function hitTestTextLabel(shape, px, py, pad = 8) {
  const fontSize = shape.fontSize || 16;
  const tw = Math.max(40, (shape.text || '').length * fontSize * 0.6 + 8);
  const th = fontSize + 8;
  if (px >= shape.x - pad && px <= shape.x + tw + pad && py >= shape.y - pad && py <= shape.y + th + pad) {
    return 'body';
  }
  return null;
}
...
export function drawAnnotateShape(ctx, shape, selected = false) {
  if (shape.type === 'arrow') {
    ...
    drawEllipseFrame(ctx, shape.x, shape.y, shape.w, shape.h, selected);
  } else if (shape.type === 'text') {
    drawTextLabel(ctx, shape, selected);
  } else {
...
export function hitTestRoundedFrame(shape, px, py, pad = ANNOTATE_HIT_PAD) {
  ...
  if (nearLeft || nearRight || nearTop || nearBottom || inside) return 'body';
  return null;
}
...
export function hitTestShapes(shapes, px, py) {
  for (let i = shapes.length - 1; i >= 0; i -= 1) {
    const s = shapes[i];
    let handle = null;
    if (s.type === 'arrow') handle = hitTestArrow(s, px, py);
    else if (s.type === 'ellipse') handle = hitTestEllipseFrame(s, px, py);
    else if (s.type === 'text') handle = hitTestTextLabel(s, px, py);
    else handle = hitTestRoundedFrame(s, px, py);
