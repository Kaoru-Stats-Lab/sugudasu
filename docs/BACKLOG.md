...
- [ ] **Zenn #12** — Gemini事故カタログ × Excel深堀り · 5プリセット（[`ZENN_ARTICLE_12_DRAFT.md`](notes/ZENN_ARTICLE_12_DRAFT.md) · **7/7 予約** · [`X_POST_ZENN12_LAUNCH.md`](notes/X_POST_ZENN12_LAUNCH.md)）
...
**評価軸:** [`PRODUCT_CONSTITUTION.md`](product/PRODUCT_CONSTITUTION.md)（F1〜F7）· [`PRODUCT_IDEA_JUDGMENT_LEDGER.md`](notes/PRODUCT_IDEA_JUDGMENT_LEDGER.md) §2-2（M1〜M7）· 思想は **§8-12**  
...
- [x] LP / hub コピー: 両方ターゲット — hub 短縮 · **normalize Tier S LP**（FV · 3ステップ · FAQ · `TOOL_PAGE_LP_STRATEGY.md`）
- [ ] 共通 **スタンドアロンバッジ**（§1-15 横断UI）— normalize 単体バッジは反映済 · shell 共通は未
- [ ] Zenn: #12 続編 or note で「総務が変換サイトに貼れない」角度（任意）· **#12 本文に統合済（2026-07-01）** · CTA · normalize FAQ 要約済
...
**MVP スコープ（α v0.1.0 · 2026-07-02 着手）**

- [x] id: `mask` · productName: **SUGUDASU マスク**（副題: スクショの機密消し）
- [x] 矩形 **黒塗り / モザイク**（細かめ・粗め）· PNG ダウンロード · クリップボードコピー
- [x] スタンプ:「サンプル」「ダミー」「テスト」（テキスト · ローカル描画）
- [x] 上限: 25MB · 長辺 8192 超は縮小（`webp-to-jpg` 踏襲）
- [x] FAQ: 非送信 · スタンプ単体では機密隠し不可 · OCR/自動検出は **v2 以降**
- [x] Undo/Redo（最大20）· Ctrl+V · D&D · `scripts/mask-engine.test.mjs`

**やらない**  
顔自動検出 · クラウド共有 · PDF 多ページ一括（初期）

**SSOT**  
`docs/notes/MASK_TOOL_SPEC.md` · Zenn メモは `ZENN_WEBP_TO_JPG_DRAFT_MEMO` と同型で可（未着手）
...
- [x] id: `time-calc` · productName: **SUGUDASU 時給計算**（nav: 時給計算 · 給与計算は不採用）
...
2. **`mask`** 新規 MVP — **α v0.1.0 実装済（2026-07-02）· デプロイ待ち**  
...
- [x] **P1** `hub.html` リード — **発見専用に短縮**（2026-07-01 · `TOOL_PAGE_LP_STRATEGY.md`）· 獲得は各 `/{tool}` LP へ
- [ ] **禁止** キーワード詰め込み · 虚偽の専門性
...
### 4-5. `test-data` 件数スケール（SSOT: `TEST_DATA_TOOL_SPEC.md` §7–§8）

- [ ] **P1** v0.5 — 社員マスタ **25万** · IDオフセット · 5,000×50チャンク · **UTF-8 BOM CSV** 結合DL
- [ ] **P2** v0.5 — ZIP（単一CSVまたは part 連番）
- [ ] **P2** v0.6 — Web Worker · 給与明細大規模（75万行級）
- [ ] **P2** v0.6+ — xlsx（**≤5,000件**サンプルのみ）
- [ ] **P2** FAQ — 5,000＝1チャンク · Excel実務限界（§8.3）· 25万はCSV正本
...
- **評価:** `docs/product/PRODUCT_CONSTITUTION.md`（F1〜F7）· `docs/notes/PRODUCT_IDEA_JUDGMENT_LEDGER.md` §6（本件の位置づけ）· §2-2（市場）
...
**Status:** 要件定義（P2）  
**仕様 SSOT:** [`docs/notes/SHIFT_SCOPE_AND_V2_SPEC.md`](notes/SHIFT_SCOPE_AND_V2_SPEC.md)（スコープ · v2 Must · Non-goal · FB 募集）

- [ ] **繁忙期 · 繁忙日の人数増**（曜日 · 特定日 · 期間 — §5-1 V2-7）
- [ ] 1時間帯あたり **複数スタッフ**（データモデル: 人[] · §5 V2-1）
- [ ] **ポジション行**（キッチン/ホール等 · §5 V2-2）
- [ ] **新人のみの帯を禁止**（帯単位。現行「重複NGキーワード」は同一日2新人回避で別ロジック）
- [ ] 自動生成が対象か / 手動のみか — 要件確定（§5 V2-6）
- [ ] 印刷レイアウト（セル内複数行）
- [ ] UI 用語「枠」→ 時間帯/ポジション（§4）
- [x] `shift.html` FAQ — 向いている業態名サジェスト · アンマッチ誠実線（§4 · JSON-LD）
...
- **IA FIX（2026-07-01）:** 別URL LP は作らず **`/{tool}` = LP + 本体**。hub は発見専用 — 正本 [`TOOL_PAGE_LP_STRATEGY.md`](notes/TOOL_PAGE_LP_STRATEGY.md)
