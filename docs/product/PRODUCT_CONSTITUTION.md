...
横連携は、既存コア仕事（帳票・名簿・画像の隠す/切る・テキスト整形・会議の持ち帰り）の直前・直後に限る。Hub にあること自体は GO 理由にしない（[`CASE-2026-009`](../legal/CASE_LAW.md#case-2026-009)）。
...
- PCブラウザだけで十分成立する仕事を扱う（Pain がスマホで起きても、ブラウザ完結かつ PC でも同じ仕事が終わるならコア可。ネイティブ必須・Hub のモバイルファースト化・「スマホだから足す／足さない」は不可。[`CASE-2026-009`](../legal/CASE_LAW.md#case-2026-009)）
