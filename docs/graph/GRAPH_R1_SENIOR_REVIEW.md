...
- ~~色変更 UI~~ → **読み替え:** **ロール色は HEX＋ピッカーで必須**。禁止は **全要素ピッカー** と **テーマ一式エディタ** のみ（[`GRAPH_R1_COLOR_MUST_JUDGMENT.md`](./GRAPH_R1_COLOR_MUST_JUDGMENT.md)）
- 線幅の自由スライダーUI
- 凡例編集
- 軸ラベル編集
- ドラッグ編集
- グラフエディタ
- テーマエディタ（色＋フォント＋余白の一式）

品質基準は「編集できないこと」ではなく:

> **編集しなくても、そのまま資料に貼れる**  
> 色は出力前に **HEX / ピッカーでロール単位**に変えられること（貼付後は直せない）。
特に **SVG 貼付後は編集できない（2026-08-14 確定）**。  
**編集できない前提で作る**方が、Office 各版・OpenOffice / LibreOffice まで含めて汎用的。  
MS-Office とオープン系スイートは挙動が違うので、MS の「図形に変換」等に最適化しない。  
余白・ラベル・線幅などのデフォルト欠陥は、編集UI需要ではなく **出荷品質の欠陥** として扱う。  
「PPTなら直せる」「365なら直せる」は一般解にしない。
