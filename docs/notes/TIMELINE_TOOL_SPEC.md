...
| O4 | コピー成功 | `sg-copy-flash` + `コピーしました` + トースト（§3.8） |
...
### 7-5. Sync ビューレーン（提督採用 · 2026-06-25）

**調査正本:** `docs/notes/timeline-sync-lanes-gemini-RESULT.md`  
**結論:** 正本データは **1本**（`TimelineState` · 行ID・時刻連動は共通）。**2レーン以上必要なのは「表示」** — 別タイムライン（案C）は採用しない。

#### ビュープロファイル（URL またはルーム設定で切替）

| ビュー | 対象 | 表示 | 編集 |
|--------|------|------|------|
| **crew** | 運営デスク · 進行D · 音響（クルー） | 全列 · `crewNote` · 時刻 · 話者 | ◎（編集者ロール） |
| **stage** | 司会 · MC · ステージモニター | `publicTitle` · 話者 · 残り時間 · 次コマ · **今**線 | ×（閲覧 · 手動反映） |
| **public** | 受付 · 参加者モニター（S2+） | 時刻 · `publicTitle` のみ · 現行/次の1件 | × |

#### 行スキーマ拡張（Sync · 案B）

| フィールド | 層 | 説明 |
|------------|-----|------|
| `title` | コア+Sync | 幹事の作業用タイトル（従来どおり） |
| `publicTitle` | **Sync** | 司会・来場者向け表示名。未設定時は `title` をフォールバック |
| `note` | コア+Sync | コアでは運営メモ。Sync では **`crewNote` 相当** — **stage/public では非表示** |
| `speaker` | Sync S2+（任意） | 登壇者名 · stage でタイトル横に優先表示 |

**コア無料ライン:** `publicTitle` / `speaker` は **持たない**（schema 追加なし · F1 維持）。  
**エンジン:** `recalcTimeline` は時刻のみ — 表示フィールドは UI 層でフィルタ。

#### やらないこと（調査で確定）

- 2本の独立タイムラインの双方向同期（スプレッドシートの行コピー地獄と同型）
- Shoflo 型 **役割別マルチ列** を MVP/S2 で一気に入れる（`crewNote` + ビューで足りる）
...
- [ ] Sync **stage** ビュー: `crewNote` が一切出ない · `publicTitle` 表示
- [ ] Sync **crew** ビュー: `publicTitle` と `note` の両方編集可
