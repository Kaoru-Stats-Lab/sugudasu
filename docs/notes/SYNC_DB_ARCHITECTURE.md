...
**更新:** 2026-06-25（Gemini 調査採否反映）  
**調査:** [`sync-db-architecture-gemini-RESULT.md`](sync-db-architecture-gemini-RESULT.md)  
**関連:** `SYNC_S1_ARCHITECTURE.md` · `SYNC_RETENTION_POLICY.md` · `SYNC_STORAGE_QUOTAS.md`
...
| ファイル | 内容 |
|----------|------|
| `20260625_sync_s1.sql` | rooms + states |
| `20260625_sync_retention.sql` | `retain_until` |
| `20260625_sync_quotas.sql` | トリガー |
| `20260626_sync_billing_layer.sql` | profiles · entitlements · `product_type` · drop `stripe_customer_id` on room |
| `20260626_sync_event_public_id.sql`（未作成） | `event_public_id` 追加 · unique · prefix check |
| `20260626_sync_room_unify.sql`（未作成） | payload/revision を room に移し states 廃止 |

**Supabase 未適用なら** s1 を書き直して1ファイル化も可。
