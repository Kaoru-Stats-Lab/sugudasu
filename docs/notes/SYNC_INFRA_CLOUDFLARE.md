...
**更新:** 2026-06-26（Supabase 結合完了 · `/timeline/app/` 本番確認）  
...
> **Agent / 別セッション:** 製品仕様は [`SUGUDASU_SYNC_LINE.md`](SUGUDASU_SYNC_LINE.md) · **環境変数**は [`SYNC_ENV_KEYS.md`](SYNC_ENV_KEYS.md) · コアデプロイは [`DEPLOY_CLOUDFLARE_PAGES.md`](DEPLOY_CLOUDFLARE_PAGES.md) · **台帳**は [`DEPLOY_LOG.md`](DEPLOY_LOG.md)（`target=sync`）· Backlog **§5-4**
...
**Sync Supabase 変数:** [`SYNC_ENV_KEYS.md`](SYNC_ENV_KEYS.md) — **2026-06-26 本番注入済み**
...
### 2-2. Supabase 結合・本番確認（2026-06-26）

| 項目 | 状態 |
|------|------|
| Sync 専用 Supabase Project | Done（ASL 分離） |
| RLS 有効 · マイグレーション 4 本 | Done |
| CF `SUGUDASU_*` / `SUPABASE_*` 環境変数 | Done（service_role は Encrypt） |
| `dist-sync/data/sync-public-config.json` | anon のみ |
| 本番 `sync.sugudasu.com/timeline/app/` | ログインフォーム表示 · env エラー解消 |

手順再現 · 変数一覧: [`SYNC_ENV_KEYS.md`](SYNC_ENV_KEYS.md)  
製品受け入れ（ルーム保存 E2E）: [`SYNC_S1_ARCHITECTURE.md`](SYNC_S1_ARCHITECTURE.md) §5 — **未完了**
...
**必須:** [`DEPLOY_LOG.md`](DEPLOY_LOG.md) に `target: sync` · `status: approved` を追記してから実行。

```powershell
cd C:\asl_dev\sugudasu

# ゲート + build:pages:sync + 月次予算 + wrangler（一括）
npm run deploy:pages:sync
```

内訳: `release:pages:sync`（`deploy:gate:sync` · ビルド · budget consume）→ `wrangler pages deploy`
