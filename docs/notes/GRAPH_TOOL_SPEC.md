...
| Waterfall | `docs/graph/GRAPH_WATERFALL_SPEC.md` |
...
2. Intent をカード＋ミニ図で選ぶ（種類名ではなく目的）— αでプレビュー可能なのは推移・比較・順位・達成・**単位の違う2指標**・**増減要因（Waterfall）**
...
**期待合わせ:** Intent 横のミニ図は装飾見本（Renderer出力ではない）。本番プレビューは常に実パイプライン。内訳は準備中。**増減要因（BRIDGE）は Waterfall で α 対応**（開始・増減・終了の行ラベルが必要）。
...
- Pie / Donut（R3 HOLD）· 汎用 Stacked（目標以外）
