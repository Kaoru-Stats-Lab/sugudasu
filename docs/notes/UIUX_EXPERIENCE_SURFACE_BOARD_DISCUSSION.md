...
### S-PILOT-2（2026-08-05 · 仮説検証）

| 項目 | 内容 |
|------|------|
| 対象 | `pdf-fill`（画面証拠あり · visual_workbench） |
| 仮説 | 記入キャンバスを唯一の Workspace にすれば「どこで書くか」が一目で立つ |
| 実装 | `.sg-surface-workspace` でツールバー+キャンバスを包む · 「提出用PDFを完成」は `.sg-surface-section` |
| やらない | 色変更 · 全ツール横展開 · FAQ/リード改稿 · ドロップ面の格下げ |
| 判定 | 提督が `/pdf-fill` で PDF を開いたあと、記入面が他カードより一段強く見えるか |
...
**許可:** Hub 検索の一段上げ（S-PILOT）· **pdf-fill Workspace 仮説検証（S-PILOT-2）のみ**。完了後に判断ログへ結果を追記してから横展開票。
