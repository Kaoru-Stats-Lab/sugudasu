# トップ検索辞書 — Gemini 生成プロンプト v2（正本）

| 項目 | 値 |
|------|-----|
| **版** | v2（正本 · 運用中） |
| **前身** | [search-dictionary-prompt-v1.md](search-dictionary-prompt-v1.md) |
| **反映先** | `data/search-dictionary/{toolId}.json` |
| **バンドル** | `npm run build:hub-search` → `data/hub-search-bundle.json` |
| **補助** | `data/synonyms.json` · `data/tool-intent-map.json` |

**用途:** [sugudasu.com](https://sugudasu.com/) の辞書ベース検索用。Embedding / AI 検索ではない。  
...
Hub カード順で 1 本ずつ投げる。終わったら「次」と指示。
