...
**法務・サイトページ**（privacy / terms / hub / updates / statements）= プロダクト接頭辞なし可。`productName` をそのままヘッダーに。**レイアウト・タイポ**: [`DESIGN_GUIDELINE_INFO_PAGES.md`](DESIGN_GUIDELINE_INFO_PAGES.md)
6. **新規ツール追加時** — **`docs/notes/TOOL_NAMING_AGENT_PLAYBOOK.md`** の手順 A（registry 先 → HTML · hub · shell → `npm run validate:tool-naming`）。値の SSOT は `data/tool-registry.json`。
...
| normalize | 文字列正規化 | SUGUDASU 正規化 | 正規化 |
...
命名の3層は **§1.3** · SSOT は `data/tool-registry.json`。`data-sg-title` = `productName` · ナビ = `navLabel`。
